...
std::string parse_error::message_with_err_loc() const
{
	// line numbers start from 1
	int lines = std::count(text_start_, error_position_, '\n') + 1;
	...
	auto pos = error_position_;
	for(; pos != text_start_ && *pos != '\n'; pos--);
	return formatter()
		<< "line " << lines
		<< ", character " << utf8::size(pos, error_position_)
		<< ": " << message;
}
